...
#!/bin/bash
# Audited Contract Arbitrum One Deployment Script
# This script deploys your security-analyzed Stylus contract to Arbitrum One
...
# Check if we can connect to Arbitrum One
echo "Checking connection to Arbitrum One..."
if ! curl -s -X POST -H "Content-Type: application/json" \
  --data '{"jsonrpc":"2.0","method":"net_version","params":[],"id":1}' \
  "${RPC_URL}" > /dev/null; then
    echo "Error: Cannot connect to Arbitrum One RPC"
    exit 1
fi
echo "Connected to Arbitrum One!"
...
echo "Deployment completed successfully on Arbitrum One!"
